...
코드 파일을 수정할 때마다 즉시 커밋 & 푸시한다. 숙제.md / 수정히스토리.md 갱신 시점과 무관하게 코드 변경은 바로바로 반영한다.
...
1. `git pull` 로 최신 코드를 가져온다
2. 충돌이 있으면 사용자에게 알리고 해결 방법을 안내한다
3. pull 완료 후 `숙제.md` 를 읽고 맥락을 파악한다
